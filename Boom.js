...
(function(window, undefined) {
	...
	// 计算坐标并添加新的层覆盖在原图上
	// 返回一个新的 jQuery 对象（dom结点，大小及相对坐标与原图一致）
	function calcPosition(elem) {
		imgUrl = elem.attr('src');

		var obj = elem[0];

		// getBoundingClientRect 方法用于计算 elem 相对于浏览器左上角的 4 个点的坐标
		// 还有 getBoundingClientRect 是 JS 对象方法，注意下文 jQuery 对象和 JS 对象的相互转换
	...
	// 计算 boom 动画轨迹终止点
	function boomRamdomPosition(center, div) {
		var
		// 直线斜率 
			slope = 0,
			// 爆炸范围
			...
			elem
				...
				.queue('shake', function() {
					...
					for (; i < length; i++) {
						...
						var resultPoint = boomRamdomPosition(certerPonit, divPoint);
